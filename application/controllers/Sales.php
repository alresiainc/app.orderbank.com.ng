...
class Sales extends MY_Controller {
	...
	public function ajax_list()
	{
		...
		foreach ($list as $sales) {
	        ...
	        $row[] = get_warehouse_name($sales->warehouse_id);
			...
			$row[] = ucfirst($sales->created_by);
					...
					$str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											$str2.='<li>
												<a title="Take Print" target="_blank" href="print_invoice/'.$sales->id.'">
    ...
    // Agent-name-Today’s-date-Sales-invoice
        // $dompdf->stream(str_replace('---', '-', str_replace(' ', '-', strtolower($customer_name)))."-".date('M')."_".date('d')."_".date('Y')."-sales-invoice", array("Attachment"=>0));
	}
