...
// 		$this->db->select("a.*,b.c`ategory_name");
		...
		if($q1->num_rows()>0){
			$i=0;
			$tot_expense_amt=0;
			$expenses = '<h4 class="font-weight-bolder text-dark" style="margin-top: 0px; margin-bottom: 0px; padding: 10px 15px; font-weight:bold;">Expenses</h4>';
			$expenses .= '<ul class="list-group  " style="margin-bottom: 0px!important;">';
			
		
			$result = json_decode(json_encode($q1->result()), true);
			// Remove duplicate value according to 'name'
			
			$catIds = array();
            $myarray = array_filter($result, function($el) use (&$catIds) {
                if (in_array($el['category_name'], $catIds)) { // if the id has already been seen
                    return false; // remove it
                } else {
                    $catIds[] = $el['catcategory_nameid']; // the id has now been seen
                    return true; // but keep the first occurrence of it
                }
            });
            $result_unique_arr = $this->unique_key($result,'category_name');
				// var_dump($result_unique_arr);
				// var_dump($q1->result());
				$count_array = count($result);
				
				foreach (json_decode(json_encode($result_unique_arr)) as $res1) {
// 			foreach ($q1->result() as $res1) {
			    $this->db->where("(a.expense_date>='$from_date' and a.expense_date<='$to_date')");
			    $this->db->where("a.category_id",$res1->category_id);
                $this->db->from("db_expense as a");
                // $this->db->from("db_expense_category as b");
                $category=$this->db->get();
                $category_count = $category->num_rows();
                
                 $total_amt_of_items_in_category = 0;
                 $count_item_in_category = 0;
                foreach ($category->result() as $category_result) {
                    $total_amt_of_items_in_category +=$category_result->expense_amt;
                    $count_item_in_category++;
                }
                // $category_count = $total_amt_in_cat;
                //  $category_count = $res1->expense_amt;
               
                //  $category_count = $q1->num_rows().'-'.$count_array;
                
                 $total_amt_of_items_in_category = $total_amt_of_items_in_category;
                 $count_item_in_category = $count_item_in_category;
				/*******************
				...
				$expenses .= '<li class="list-group-item" style="margin-bottom:2px;border:0px; background-color: #f1f5f8;" >'.$res1->category_name.' - <lable class="" style="/* background-color: #cfc9c9; *//* padding: 3px; *//* border-radius: 6px; *//* font-size: 9px; */">'.$count_item_in_category.'</lable> <span class="badge bg-white" style="background-color: rgb(255 255 255 / 0%); color: #000;">'.store_number_format($total_amt_of_items_in_category).'</span></li>';
				
				// $tot_expense_amt+=$res1->expense_amt;
				$tot_expense_amt+=$total_amt_of_items_in_category;
        ...
        //             $expenses .= '<li class="list-group-item" style="margin-bottom:2px;border:0px; background-color: #f1f5f8;" >'.$res1->expense_code.' <span class="badge bg-white" style="background-color: rgb(255 255 255 / 0%); color: #000;">'.store_number_format($value->expense_amt).'</span></li>';
				    // $tot_expense_amt+=$value->expense_amt;
        //         }
        //     }
			...
			$expenses .= '</ul>';
			$expenses .= '<h4 class="p-5 text-uppercase font-weight-bolder bg-info" style="border: 2px #000 solid;border-left: 0px; border-right: 0px;  padding: 10px 15px; margin-top: 0px; margin-bottom: 0px;  background:#3c78d8;color:white;">Total Expenses<div style="float:right;">'.store_number_format($tot_expense_amt).'</div></h4>';
			$expenses_grand_total = $tot_expense_amt;
			// echo "<tr>
			...
			$expenses = '<h4 class="font-weight-bolder text-dark" style="margin-top: 0px; margin-bottom: 0px; padding: 10px 15px; font-weight:bold;">Expenses</h4>';
			$expenses .= '<ul class="list-group  " style="margin-bottom: 0px!important;">';
			$expenses .= '<li class="list-group-item" style="margin-bottom:2px;border:0px; background-color: #f1f5f8;" >No Records Found <span class="badge bg-white" style="background-color: rgb(255 255 255 / 0%); color: #000;">---</span></li>';
				
			$expenses .= '</ul>';
			$expenses .= '<h4 class="p-5 text-uppercase font-weight-bolder bg-info" style="border: 2px #000 solid;border-left: 0px; border-right: 0px;  padding: 10px 15px; margin-top: 0px; margin-bottom: 0px;  background:#3c78d8;color:white;">Total Expenses<div style="float:right;">'.store_number_format(0).'</div></h4>';
		...
		if($q1->num_rows()>0){
			...
			$tot_paid_amount=0;
			$tot_due_amount=0;
			$purchased = '<h4 class="-weight-bolder text-dark" style="margin-top: 0px; margin-bottom: 0px; padding: 10px 15px; font-weight:bold;">Cost of Goods Purchased </h4>';
			$purchased .= '<ul class="list-group  " style="margin-bottom: 0px!important;">';
			...
			foreach ($q1->result() as $res1) {
				...
				$purchased .= '<li class="list-group-item" style="margin-bottom:2px;border:0px; background-color: #f1f5f8;" >'.$res1->purchase_code.' <span class="badge bg-white" style="background-color: rgb(255 255 255 / 0%); color: #000;">'.store_number_format($res1->grand_total).'</span></li>';
			...
			$purchased .= '</ul>';
			$purchased .= '<h4 class="p-5 text-uppercase font-weight-bolder bg-info" style="border: 2px #000 solid;border-left: 0px; border-right: 0px;padding: 10px 15px; margin-top: 0px; margin-bottom: 0px;  background:#3c78d8;color:white;">Total Cost of Goods Purchased<div style="float:right;">'.store_number_format($tot_grand_total).'</div></h4>';
			$purchased_grand_total = $tot_grand_total;
			// echo "<tr>
			// 		  <td class='text-right text-bold' colspan='$total_columns_count'><b>Total :</b></td>
			// 		  <td class='text-right text-bold'>".store_number_format($tot_grand_total)."</td>
			// 		  <td class='text-right text-bold'>".store_number_format($tot_paid_amount)."</td>
			...
			$purchased .= '<li class="list-group-item" style="margin-bottom:2px;border:0px; background-color: #f1f5f8;" >No Records Found<span class="badge bg-white" style="background-color: rgb(255 255 255 / 0%); color: #000;">'.store_number_format(0).'</span></li>';
			$purchased .= '</ul>';
			$purchased .= '<h4 class="p-5 text-uppercase font-weight-bolder bg-info" style="border: 2px #000 solid;border-left: 0px; border-right: 0px;padding: 10px 15px; margin-top: 0px; margin-bottom: 0px;  background:#3c78d8;color:white;">Total Cost of Goods Purchased<div style="float:right;">'.store_number_format($tot_grand_total).'</div></h4>';
	...
	if($q1->num_rows()>0){
		...
		$tot_paid_amount=0;
		$tot_due_amount=0;
		$sales = '<h4 class="-weight-bolder text-dark" style="margin-top: 0px; margin-bottom: 0px; padding: 10px 15px; font-weight:bold;">Revenue</h4>';
		$sales .= '<ul class="list-group  " style="margin-bottom: 0px!important;">';
		...
		foreach ($q1->result() as $res1) {
			...
			$sales .= '<li class="list-group-item" style="margin-bottom:2px;border:0px; background-color: #f1f5f8;" >'.$res1->sales_code.'<span class="badge bg-white" style="background-color: rgb(255 255 255 / 0%); color: #000;">'.store_number_format($res1->grand_total).'</span></li>';
		...
		$sales .= '<h4 class="p-5 text-uppercase font-weight-bolder bg-info" style="border: 2px #000 solid;border-left: 0px; border-right: 0px;padding: 10px 15px; margin-top: 0px; margin-bottom: 0px;  background:#3c78d8;color:white;">Total Revenue Generated<div style="float:right;">'.store_number_format($tot_grand_total).'</div></h4>';
		$sales_grand_total = $tot_grand_total;
	}
	else{
		$total_columns_count=8;
		if(store_module() && is_admin()){
		...
		$sales .= '<h4 class="-weight-bolder text-dark" style="margin-top: 0px; margin-bottom: 0px; padding: 10px 15px; font-weight:bold;">Revenue</h4>';
		$sales .= '<ul class="list-group  " style="margin-bottom: 0px!important;">';
        $sales .= '<li class="list-group-item" style="margin-bottom:2px;border:0px; background-color: #f1f5f8;" >No Records Found <span class="badge bg-white" style="background-color: rgb(255 255 255 / 0%); color: #000;">---</span></li>';
        $sales .= '</ul>';
		$sales .= '<h4 class="p-5 text-uppercase font-weight-bolder bg-info" style="border: 2px #000 solid;border-left: 0px; border-right: 0px;padding: 10px 15px; margin-top: 0px; margin-bottom: 0px;  background:#3c78d8;color:white;">Total Revenue Generated<div style="float:right;">'.store_number_format(0).'</div></h4>';
		...
		$return = '<h5 class="font-weight-bolder text-center " style="font-weight:bold;">For the Following Period ['.date('M j', strtotime($from_date)).'], '.date('Y', strtotime($from_date)).' and ['.date('M j', strtotime($to_date)).'], '.date('Y', strtotime($to_date)).' </h5>';
		$return .= $sales;
		$return .= $purchased;
		//Groos Profit
		$groos_profit = ($sales_grand_total-$purchased_grand_total);
		$return .= '<h4 class="-weight-bolder text-dark" style="border-bottom: 3px #000 solid;margin-top: 0px; margin-bottom: 0px; padding: 10px 15px; font-weight:bold;  background:#f8d965;color:black;">Groos Profit<div style="float:right;">'.store_number_format($groos_profit).'</div></h4>';
		$return .= $expenses;

		$net_profit = ($groos_profit-$expenses_grand_total);
		$return .= '<h4 class="-weight-bolder text-dark" style="border-bottom: 3px #000 solid;margin-top: 0px; margin-bottom: 0px; padding: 10px 15px; font-weight:bold;  background:#6fb451;color:white;">Net Profit<div style="float:right;">'.store_number_format($net_profit).'</div></h4>';
