...
class Users_model extends CI_Model
{
	...
	public function verify_and_save($data)
	{
		...
		if (isset($_POST['states'])) {
			...
			$states_list = sizeof($_POST['states']);
			foreach ($_POST['states'] as $res2 => $val2) {
				$states_info = array('user_id' => $user_id, 'state_id' => $val2);
				$q2 = $this->db->insert("db_userstates", $states_info);
				if (!$q2) {
					return "failed";
				}
			}
		...
		$this->session->set_flashdata('success', 'Success!! New User created Successfully!!');
		return "success";
	}
	public function verify_and_update($data)
	{
		...
		if (isset($_POST['states'])) {
			$this->db->where('user_id', $q_id)->delete("db_userstates");
			$states_list = sizeof($_POST['states']);
			foreach ($_POST['states'] as $res => $val) {
				$states_info = array('user_id' => $q_id, 'state_id' => $val);
				$q2 = $this->db->insert("db_userstates", $states_info);
				if (!$q2) {
					return "failed";
				}
	...
	public function status_update($id, $status)
	{
		if ($id == $this->session->userdata('inv_userid')) {
			echo "You Can't Diactivate Yourself!";
			exit();
		}
