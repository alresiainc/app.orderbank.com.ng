...
class Dashboard_model extends CI_Model
{
	...
	public function breadboard_values()
	{
		...
		///Find total stock transfer
		/*if(store_module() && is_admin()){if(!empty($store_id)){ 
					$this->db->where("store_id",$store_id);}
				}else{ */
		$this->db->where("store_id", get_current_store_id());
		/*}*/
		$this->get_by_date('transfer_date'); //DATES FUNCTION
		$this->db->select("coalesce(count(*),0) as tot_stock_trns");
		$this->db->from("db_stocktransfer");
		$this->db->where("status=1");

		$tot_stock_trns = $this->db->get()->row()->tot_stock_trns;
		$info['tot_stock_trns'] = $tot_stock_trns;
		...
		///Find Total Sales Payment
		/*if(store_module() && is_admin()){if(!empty($store_id)){ 
					$this->db->where("store_id",$store_id);}
				}else{ */
		$this->db->where("store_id", get_current_store_id());
		/*}*/
		$this->get_by_date('payment_date'); //DATES FUNCTION

		$this->db->select("coalesce(count(*),0) as tot_sale_payment");
		$this->db->from("db_salespayments");
		$this->db->where("status=1");
		$tot_sale_payment = $this->db->get()->row()->tot_sale_payment;
		...
		// $this->db->where("status=1");
		$tot_orders_generated = $this->db->get()->row()->tot_orders_generated;
		$info['tot_orders_generated'] = $tot_orders_generated;
		...
		$this->db->where("store_id", get_current_store_id());
		/*}*/
		$this->get_by_date('created_date'); //DATES FUNCTION
		...
		$info['total_purchase'] = $CI->currency(kmb($total_purchase));
