...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <script>
        ...
        function handleStatus(status, id) {
            $('#change-order-status-modal .modal-title').text(toTitleCase(status.replace(/_/g, ' ')))

            if (status === 'not_answering_call' || status === 'cancelled' || status === 'out_of_area' || status ===
                'duplicated') {

                $('#change-order-status-form')[0].reset();
                $('#order_id').val(id);
                $('#status').val(toTitleCase(status.replace(/_/g, ' ')));
                // Confirm with the user before sending AJAX request
                if (confirm("Are you sure you want to switch to " + toTitleCase(status.replace(/_/g, ' ')) + "?")) {
                    process_selected()
                }
            } else if (status === 'delivered') {

                // Show model to gather additional details for delivered status
                $('#change-order-status-modal').modal('show');
                $('#change-order-status-form')[0].reset();
                $('#order_id').val(id);
                $('#status').val(toTitleCase(status.replace(/_/g, ' ')));

                $('#amount').prop('disable', false).parents('.form-row').removeClass('hidden');
                $('#quantity').prop('disable', false).parents('.form-row').removeClass('hidden');


                $('#change-order-status-form-button').click(function(e) {
                    e.preventDefault();

                    if (check_field("fees") == true && check_field("amount") == true && check_field("quantity") ==
                        true) {
                ...
                // Show model to input fees for canceled status
                $('#change-order-status-modal').modal('show');
                $('#change-order-status-form')[0].reset();
                $('#order_id').val(id);
                $('#status').val(toTitleCase(status.replace(/_/g, ' ')));

                $('#amount').prop('disable', true).parents('.form-row').addClass('hidden');
                $('#quantity').prop('disable', true).parents('.form-row').addClass('hidden');


                $('#change-order-status-form-button').click(function(e) {
                    e.preventDefault();
                    if (check_field("fees") == true) {
                        toastr["warning"]("You have Missed Something to Fillup!");
                        return;
                    }
        ...
        function process_selected() {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo site_url('orders/change_order_status') ?>",
                data: data,
                cache: false,
                contentType: false,
                processData: false,
                success: function(result) {
                    ...
                    if (result == "success") {
                        ...
                        $('#change-order-status-modal').modal('toggle');
        ...
        function load_datatable() {
            ...
            var table = $('#order_table').DataTable({
                ...
                "ajax": {
                    "url": "<?php echo site_url('orders/out_for_delivery_order_json_data') ?>",
                    "type": "POST",
                    "data": {
        ...
        $(document).ready(function() {
            ...
            $('#new-order-form-button').click(function(e) {
                ...
                if (check_field("shopify_id") == true || check_field("product_id") == true || check_field(
                        "country") == true || check_field("order_date") == true) {
                    toastr["warning"]("You have Missed Something to Fillup!");
                    return;
                }
