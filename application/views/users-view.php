              ...
              <div class="box-body">
                <table id="example2" class="table table-bordered custom_hover" width="100%">
                  ...
                  <tbody>
                    ...
                    $q1 = $this->db
                      ->select('a.id as user_id, a.username, GROUP_CONCAT(d.state SEPARATOR ", ") as states') // Replace `state_name` with the column in `db_states`
                      ->from('db_users as a')
                      ->join('db_roles as b', 'a.role_id = b.id', 'left') // Example join condition
                      ->join('db_userstates as c', 'a.id = c.user_id', 'left') // Join `db_userstates`
                      ->join('db_states as d', 'c.state_id = d.id', 'left') // Join `db_states`
                      ->group_by('a.id') // Group by user ID to combine states
                      ->order_by('a.id', 'desc')
                      ->get();
